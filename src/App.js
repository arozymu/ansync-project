...
export class Clock extends React.Component {
    ...
    render () {
        ...
        return (
            <svg height={size} width={size}>
                ...
                <g className={'timepicker-hour'}>
                    {this.renderHours()}
                </g>
                
                <g className={'timepicker-minute'}>
                    {this.renderMinutes()}
                </g>
            </svg>
        );
    ...
    renderHours () {
        ...
        for (let index = 0; index < positions.length; ++index) {
            ...
            values.push(
                <g
                    className={`timepicker-value${hours === hour ? ' active' : ''}`}
                    key={index}
                    onClick={onClick}
    ...
    renderMinutes () {
        ...
        for (let minute = 0; minute < positions.length; ++minute) {
            ...
            values.push(
                <g
                    id={'minute'}
                    className={`timepicker-value${minute % 5 ? ' small' : ''}${minutes === minute ? ' active' : ''}`}
                    key={minute}
                    onClick={onClick}
                    onMouseMove={onMouseMove}
                >
